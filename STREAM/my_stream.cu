...
__global__ void tuned_STREAM_Copy( double  * __restrict__ d_a , double * __restrict__ d_c );
__global__ void tuned_STREAM_Scale( double * __restrict__ d_b , double * __restrict__ d_c , double scalar);
__global__ void tuned_STREAM_Add( double   * __restrict__ d_a , double * __restrict__ d_b , double * __restrict__ d_c );
__global__ void tuned_STREAM_Triad( double * __restrict__ d_a , double * __restrict__ d_b , double * __restrict__ dc , double scalar);
#endif
...
#pragma omp parallel for
    ...
    cudaMalloc((void**)&d_a,array_elements*sizeof(double));
    cudaMalloc((void**)&d_b,array_elements*sizeof(double));
    cudaMalloc((void**)&d_c,array_elements*sizeof(double));

    cudaMemcpy(d_a,a,sizeof(double)*array_elements,cudaMemcpyHostToDevice);
 ...
 //   stat = cublasCreate(&handle);
    ...
    for (k=0; k<NTIMES; k++) {
        ...
        times[0][k] = MPI_Wtime();
#ifdef TUNED
        tuned_STREAM_Copy<<<dimGrid, dimBlock>>>(d_a, d_c);
        printf("tune\b");
#else
#ifdef _OPENMP
#pragma omp parallel for
        ...
        times[1][k] = MPI_Wtime();
#ifdef TUNED
        tuned_STREAM_Scale<<<dimGrid, dimBlock>>>(d_b, d_c, scalar);
#else
#ifdef _OPENMP
#pragma omp parallel for
#endif
        ...
        times[2][k] = MPI_Wtime();
#ifdef TUNED
        tuned_STREAM_Add<<<dimGrid, dimGrid>>>(d_a, d_b, d_c);
#else
#ifdef _OPENMP
#pragma omp parallel for
#endif
        ...
        times[3][k] = MPI_Wtime();
#ifdef TUNED
        tuned_STREAM_Triad<<<dimGrid, dimBlock>>>(d_a, d_b, d_c, scalar);
#else
#ifdef _OPENMP
#pragma omp parallel for
#endif
...
#endif
//  for (j=0; j<array_elements; j++)
    d_c[idx] = d_c[idx]+d_b[idx];
}

__global__ void tuned_STREAM_Triad( double * __restrict__ d_a, double * __restrict__ d_b, double * __restrict__ d_c, double scalar)
{
  int idx = threadIdx.x + blockIdx.x * blockDim.x;
#ifdef _OPENMP
#pragma omp parallel for
