...
/*        Copy A' to WORK in the appropriate order & Send it */
	...
	for (j1 = 0; j1 <= i__2; ++j1) {
        ...
        cudaMemcpy(d_a,   &a[kia + 1 + (kja +1)*a_dim1], m*n*sizeof(double), cudaMemcpyHostToDevice);
        cudaMemcpy(d_c, &c__[kic + 1 + (kjc +1)*c_dim1], m*n*sizeof(double), cudaMemcpyHostToDevice);
        cublasCreate(&handle);
        cublasDgemm(handle,CUBLAS_OP_T,CUBLAS_OP_N,n,m,m
