...
//        cudaMemcpy(d_a, &mem[ipa-1], n*m*sizeof(double), cudaMemcpyHostToDevice);
//        cudaMemcpy(d_c, &mem[ipc-1], n*m*sizeof(double), cudaMemcpyHostToDevice);

//        stat = cublasCreate(&handle);
//        cublasDgemm(handle,CUBLAS_OP_T, CUBLAS_OP_N, n, m, m, &d_One, d_a, lda, d_c, ldc, &d_One, d_c, ldc);

        pdtrans( "T", &m, &n, &mb, &nb, &mem[ipa - 1], &lda, &d_One, &mem[ipc - 1], &ldc, &imrow,
                 &imcol, &mem[ipw - 1], imem );

//        cudaMemcpy(&mem[ipa-1], d_a, n*m*sizeof(double), cudaMemcpyDeviceToHost);
//        cudaMemcpy(&mem[ipc-1], d_c, n*m*sizeof(double), cudaMemcpyDeviceToHost);
//        cudaFree(d_a);
//        cudaFree(d_c);
