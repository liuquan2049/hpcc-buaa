...
MPIFFT0(HPCC_Params *params, int doIO, FILE *outFile, MPI_Comm comm, int locN,
  ...
  cufftPlan1d(&d_plan, n, CUFFT_C2C, 1);
...
#pragma omp parallel for
  ...
  //cudamalloc
  cudaMalloc((void**)&d_inout, sizeof(cufftComplex)*n);
  cudaMalloc((void**)&d_work,  sizeof(cufftComplex)*n);
  cudaMemcpy(d_inout, inout, sizeof(cufftComplex)*n, cudaMemcpyHostToDevice);
  cudaMemcpy(d_work,  work,  sizeof(cufftComplex)*n, cudaMemcpyHostToDevice);

  t2 = -MPI_Wtime();
//  fftw_mpi( p, 1, inout, work );
  cufftExecC2C(d_plan, d_inout, d_work, CUFFT_FORWARD);
  cudaThreadSynchronize();
  t2 += MPI_Wtime();
...
//    HPCC_fftw_mpi( ip, 1, inout, work );
    cufftExecC2C(d_plan, d_inout, d_work, CUFFT_INVERSE);
    cudaThreadSynchronize();
    t3 += MPI_Wtime();
  ...
  //cudadestory
  cudaMemcpy(inout, d_inout, sizeof(cufftComplex)*n, cudaMemcpyDeviceToHost);
  cudaMemcpy(work,  d_work,  sizeof(cufftComplex)*n, cudaMemcpyDeviceToHost);
